...
function failTestOnConsoleError() {
  ...
  console.error = function (message) {
    const allowedPattern = ERROR_PATTERNS_WE_SHOULD_FIX_BUT_ALLOW.find(
      (pattern) => message.indexOf(pattern) > -1
    )
...
function failTestOnConsoleWarn() {
  ...
  console.warn = function (message) {
    const allowedPattern = WARNING_PATTERNS_WE_SHOULD_FIX_BUT_ALLOW.find(
      (pattern) => message.indexOf(pattern) > -1
    )
